...
const {loginStep} = require('../pageobjects/loginStep');

test('e2e ecommerce test',async({page})=>
{  
    const addProduct = 'iphone 13 pro';
    const username = 'captainamerica@gmail.com';
    const password = 'avengers@Assemble5';
    const productList = await page.locator('.card-body');
    const loginPage = new loginStep(page);
    loginPage.goTo();
    ...
    await expect(loggedUser).toEqual(username);
